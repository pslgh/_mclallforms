"""
Edit Tab - For editing existing timesheet entries using direct edit table
"""
import os
import uuid
import datetime
...
class EditTab(QWidget):
    ...
    # Signal to notify when a timesheet is updated
    entry_updated = Signal(str)  # Emits the timesheet ID
    
    def __init__(self, parent, user_info, data_manager, entry_id=None):
        ...
        self.loaded_entry = None  # Store the loaded entry data
        ...
        # If entry_id provided, load the entry
        if entry_id:
            self.load_entry(entry_id)
        
    def load_entry(self, entry_id):
        """Load an existing timesheet entry for editing"""
        # Try to load the entry from the JSON file
        ...
        try:
            ...
            if not json_file.exists():
                QMessageBox.critical(self, "Error", "Timesheet entries file not found.")
                return False
            ...
            with open(json_file, 'r', encoding='utf-8') as f:
                ...
                for entry in entries:
                    if entry.get('entry_id') == entry_id:
                        ...
                        self.populate_form_with_entry(entry)
                        return True
                        
                QMessageBox.warning(self, "Entry Not Found", f"Timesheet entry with ID {entry_id} not found.")
                return False
                
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Error loading timesheet entry: {str(e)}")
            return False
    
    def populate_form_with_entry(self, entry):
        """Populate the form fields with data from the loaded entry"""
        # Set entry ID
        self.entry_id = entry.get('entry_id', '')
        
        # Populate Project Information
        self.po_number_input.setText(entry.get('purchasing_order_number', ''))
        self.quotation_number_input.setText(entry.get('quotation_number', ''))
        self.contract_agreement_input.setCurrentText("Yes" if entry.get('under_contract_agreement') else "No")
        self.emergency_request_input.setCurrentText("Yes" if entry.get('emergency_request') else "No")
        
        # Client info
        self.client_input.setText(entry.get('client_company_name', ''))
        self.client_address_input.setPlainText(entry.get('client_address', ''))
        self.client_rep_input.setText(entry.get('client_representative_name', ''))
        self.client_phone_input.setText(entry.get('client_representative_phone', ''))
        self.client_email_input.setText(entry.get('client_representative_email', ''))
        
        # Project info
        self.project_name_input.setText(entry.get('project_name', ''))
        self.project_description_input.setPlainText(entry.get('project_description', ''))
        
        # Engineer info
        self.engineer_name_input.setText(entry.get('service_engineer_name', ''))
        self.engineer_surname_input.setText(entry.get('service_engineer_surname', ''))
        
        # Work type
        work_type = entry.get('work_type', '')
        for i in range(self.work_type_input.count()):
            if self.work_type_input.itemText(i) == work_type:
                self.work_type_input.setCurrentIndex(i)
                break
        ...
        for time_entry in time_entries:
            ...
            date_item = QTableWidgetItem(time_entry.get('date', ''))
            start_time = time_entry.get('start_time', '')
            start_hour = int(start_time[:2]) if len(start_time) >= 2 else 0
            start_item = QTableWidgetItem(f"{start_hour:02d}:00")
            ...
            end_item = QTableWidgetItem(f"{end_hour:02d}:00")
            end_item.setData(Qt.UserRole, end_hour)
            
            rest_item = QTableWidgetItem(str(time_entry.get('rest_hours', 0)))
            desc_item = QTableWidgetItem(time_entry.get('description', ''))
            ot_item = QTableWidgetItem(str(time_entry.get('overtime_rate', '1')))
            offshore_item = QTableWidgetItem("Yes" if time_entry.get('offshore') else "No")
            travel_count_item = QTableWidgetItem("Yes" if time_entry.get('travel_count') else "No")
            travel_short_item = QTableWidgetItem("Yes" if time_entry.get('travel_short_distance', False) else "No")
            travel_long_item = QTableWidgetItem("Yes" if time_entry.get('travel_far_distance', False) else "No")
            
            # Calculate equivalent hours
            hours = end_hour - start_hour - time_entry.get('rest_hours', 0)
            ot_rate = float(time_entry.get('overtime_rate', '1'))
            equivalent = hours * ot_rate
            equiv_item = QTableWidgetItem(f"{equivalent:.1f}")
        ...
        # Load tool usage entries
        tool_entries = entry.get('tool_usage', [])
        for tool_entry in tool_entries:
            row = self.tool_table.rowCount()
            self.tool_table.insertRow(row)
            
            tool_item = QTableWidgetItem(tool_entry.get('tool_name', ''))
            amount_item = QTableWidgetItem(str(tool_entry.get('amount', 1)))
            start_date_item = QTableWidgetItem(tool_entry.get('start_date', ''))
            end_date_item = QTableWidgetItem(tool_entry.get('end_date', ''))
            days_item = QTableWidgetItem(str(tool_entry.get('total_days', 1)))
        ...
        # Add a new tool row if none exist
        if self.tool_table.rowCount() == 0:
            self.add_new_tool_row()
        
        # Load report preparation
        self.report_description_input.setPlainText(entry.get('report_description', ''))
        self.report_hours_input.setValue(entry.get('report_hours', 0))
        
        # Load service charge calculation
        currency = entry.get('currency', 'THB')
        for i in range(self.currency_input.count()):
            if self.currency_input.itemText(i) == currency:
                self.currency_input.setCurrentIndex(i)
                break
        
        self.service_rate_input.setValue(entry.get('service_hour_rate', 0))
        self.tool_rate_input.setValue(entry.get('tool_usage_rate', 0))
        self.tl_short_input.setValue(entry.get('tl_rate_short', 0))
        self.tl_long_input.setValue(entry.get('tl_rate_long', 0))
        self.offshore_rate_input.setValue(entry.get('offshore_day_rate', 0))
        self.emergency_rate_input.setValue(entry.get('emergency_rate', 0))
        self.transport_charge_input.setValue(entry.get('other_transport_charge', 0))
        self.transport_note_input.setPlainText(entry.get('other_transport_note', ''))
        
        # Set VAT and discount if they exist
        if hasattr(self, 'vat_percent_input'):
            self.vat_percent_input.setValue(entry.get('vat_percent', 7))
        if hasattr(self, 'discount_amount_input'):
            total_cost_calculation = entry.get('total_cost_calculation', {})
            self.discount_amount_input.setValue(total_cost_calculation.get('discount_amount', 0))
    ...
    def setup_ui(self):
        ...
        input_style = "background-color:#c2f5ff; color: black; padding: 4px;"
        input_client_style = "background-color: #c2f5ff; color: black; padding: 4px; min-width: 600px;"
        input_boolean_style = "background-color: #c2f5ff; color: black; padding: 4px; min-width: 50px;"
        input_worktype_style = "background-color: #c2f5ff; color: black; padding: 4px; min-width: 180px;"
        input_project_style = "background-color: #c2f5ff; color: black; padding: 4px; min-width: 600px;"
        ...
        # Add buttons to the button row layout with spacing
        button_row.addStretch(1)
        button_row.addWidget(self.save_button)
        button_row.addSpacing(20)  # Add space between buttons
    ...
    def save_changes(self):
        ...
        # Use the existing entry ID if we're editing an entry
        ...
        print(f"Data file path: {self.data_manager.data_file_path}")
                # UPDATE EXISTING ENTRY - no dependencies on data manager or other methods
        try:
            # 1. Define the output file path
            json_file_path = "data/timesheet/timesheet_entries.json"
            from pathlib import Path
            json_file = Path(json_file_path)
            print(f"[UPDATE] Updating file: {json_file.absolute()}")
            
            # 2. Ensure directory exists
            import os
            os.makedirs(json_file.parent, exist_ok=True)
            
            # 3. Check if file exists, error if it doesn't
            if not json_file.exists() or json_file.stat().st_size == 0:
                QMessageBox.critical(self, "Error", "Timesheet entries file not found or empty.")
                return
            ...
            print(f"[UPDATE] Writing data to file: {json_file.absolute()}")
            ...
            print(f"[UPDATE] Temp file created with size: {temp_file.stat().st_size} bytes")
            ...
            if json_file.exists() and json_file.stat().st_size > 0:
                with open(json_file, 'r', encoding='utf-8') as f:
                    ...
                    print(f"[UPDATE] Verification successful - file has {verify_count} entries")
                    print(f"[UPDATE] First 100 chars of file: {verify_content[:100]}...")
            else:
                print(f"[UPDATE] WARNING: File verification failed!")
            
            # Show success message
            QMessageBox.information(self, "Success", "Timesheet updated successfully.")
            ...
            # Emit entry updated signal
            print(f"Emitting entry_updated signal with ID: {timesheet['entry_id']}")
            self.entry_updated.emit(timesheet['entry_id'])
    ...
    def update_tool_summary(self):
        ...
        try:
            ...
            for row in range(row_count):
                ...
                # Process dates to handle the new format with day names
                def extract_date_part(date_text):
                    if "," in date_text:  # Format with day name (e.g., "Mon, 2025/04/24")
                        return date_text.split(", ", 1)[1] if ", " in date_text else date_text
                    return date_text
                
                # Create a unique key for this date range, stripping out the day name if present
                processed_start_date = extract_date_part(start_date)
                processed_end_date = extract_date_part(end_date)
                date_range_key = f"{processed_start_date}_{processed_end_date}"
