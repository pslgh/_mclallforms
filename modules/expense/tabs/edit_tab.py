"""
Editing tab for the expense reimbursement module.
This tab is dynamically created for editing existing expense forms.
"""
import datetime
from PySide6.QtWidgets import (
    QWidget, QVBoxLayout, QHBoxLayout, QFormLayout, QGridLayout,
    QLabel, QLineEdit, QComboBox, QDateEdit, QTableWidget, QTableWidgetItem,
    QPushButton, QGroupBox, QDoubleSpinBox, QMessageBox, QHeaderView
)
...
class EditTab(QWidget):
    """Tab for editing existing expense forms"""
    
    # Signals
    form_updated = Signal(str)  # When a form is updated (emits form ID)
    ...
    def __init__(self, parent=None, form_id=None, user_info=None, data_manager=None):
        super().__init__(parent)
        self.form_id = form_id
        self.user_info = user_info or {}
        self.data_manager = data_manager
        self.expenses = []  # Store the expense items
        
        # Create UI components
        self.setup_ui()
        
        # Load form data if ID is provided
        if form_id and data_manager:
            self.load_form_data(form_id)
        
    def setup_ui(self):
        """Create the UI components"""
        editing_layout = QVBoxLayout(self)
        ...
        self.third_currency_usd_input.setRange(0, 1000000)  # Increased maximum value to match FormTab
        ...
        # Save and Cancel buttons in a horizontal layout
        ...
        editing_layout.addLayout(button_layout)
        
    def load_form_data(self, form_id):
        """Load form data from the data manager"""
        ...
        self.project_name_input.setText(form.project_name)
        
        # Handle work location selection
        if form.work_location == "Abroad":
            self.work_location_combo.setCurrentIndex(1)  # Abroad is index 1
            self.country_combo.setEnabled(True)
            self.country_combo.setVisible(True)
            index = self.country_combo.findText(form.work_country)
            if index >= 0:
                self.country_combo.setCurrentIndex(index)
                
            # Show third currency fields for abroad work
            self.currency_widgets.setVisible(True)
            
            # Show THB to USD Rate field
            ...
            self.work_location_combo.setCurrentIndex(0)  # Thailand is index 0
            self.country_combo.setEnabled(False)
            self.country_combo.setVisible(False)
            self.currency_widgets.setVisible(False)
            
            # Hide THB to USD Rate field
            for i in range(self.currency_layout.rowCount()):
                label_item = self.currency_layout.itemAt(i, QFormLayout.LabelRole)
                field_item = self.currency_layout.itemAt(i, QFormLayout.FieldRole)
                if label_item and label_item.widget() and field_item and field_item.widget():
        ...
        self.fund_thb_input.setValue(form.fund_thb)
        ...
        # Set currency rates
        self.thb_usd_input.setValue(form.thb_usd)
        
        # Set third currency if not None
        if form.third_currency and form.third_currency != "None":
            index = self.third_currency_combo.findText(form.third_currency)
            if index >= 0:
                self.third_currency_combo.setCurrentIndex(index)
            self.third_currency_usd_input.setValue(form.third_currency_usd)
        
        # Set issued by and date
        self.issued_by_input.setText(form.issued_by)
    ...
    def update_country_currency(self, country_name):
        ...
        # Get the currency code from our mapping
        currency_code = COUNTRY_CURRENCY_MAP.get(country_name, None)
        
        if country_name == "Thailand" or not currency_code:
            self.third_currency_combo.setCurrentIndex(0)  # None
            self.currency_widgets.setVisible(False)
        else:
            # For non-Thailand countries, show the third currency options
            self.currency_widgets.setVisible(True)
            
            # Block signals to prevent triggering update events
            self.third_currency_combo.blockSignals(True)
            
            # Handle special case for "No Currency"
            if currency_code == "No Currency":
                # Default to None for No Currency countries
                ...
                if not found and currency_code not in ["None", "No Currency"]:
    ...
    def get_form_data(self):
        ...
        # Implementation similar to form_tab.py but maintaining the original form ID
        pass
        
    def cancel_editing(self):
        """Cancel editing and close the tab"""
        # Ask for confirmation if changes have been made
        # For simplicity, we always ask for confirmation
        ...
        if confirm == QMessageBox.Yes:
            # Emit signal to close the tab without saving
            self.form_updated.emit(self.form_id)
    
    def save_changes(self):
        """Save changes to the form"""
        if not self.form_id or not self.data_manager:
            QMessageBox.warning(self, "Error", "No form ID for editing.")
            return
        ...
        # Get the original form to maintain certain fields
        original_form_data = self.data_manager.get_form_by_id(self.form_id)
        if not original_form_data:
            QMessageBox.warning(self, "Error", f"Original form {self.form_id} not found.")
            return
            
        # Create form data dictionary while preserving the original ID
        form_data = {
            "id": self.form_id,  # Keep the original ID
            "project_name": self.project_name_input.text().strip(),
            "work_location": self.work_location_combo.currentText(),
            "work_country": self.country_combo.currentText() if self.work_location_combo.currentText() == "Abroad" else "Thailand",
            "fund_thb": self.fund_thb_input.value(),
            "receive_date": self.receive_date_input.date().toString("yyyy-MM-dd"),
            "thb_usd": self.thb_usd_input.value(),
            "third_currency": self.third_currency_combo.currentText() if self.work_location_combo.currentText() == "Abroad" else "THB",
            "third_currency_usd": self.third_currency_usd_input.value() if self.work_location_combo.currentText() == "Abroad" else 0,
        ...
        for expense in self.expenses:
            form_data["expenses"].append({
                ...
                "amount": expense.amount,
                "currency": expense.currency,
                "official_receipt": expense.official_receipt,
                "non_official_receipt": expense.non_official_receipt
            })
        ...
        try:
            # Convert the dictionary to ExpenseFormData object
            form_obj = ExpenseFormData.from_dict(form_data)
            
            # Save the form data using the data manager's save_form method
            # This method handles both creating new forms and updating existing ones
            self.data_manager.save_form(form_obj)
            
            # Show success message
            QMessageBox.information(self, "Success", f"Expense form updated successfully")
            
            # Emit signal that form was updated
            self.form_updated.emit(self.form_id)
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Failed to update expense form: {str(e)}")
            # print("Error", f"Failed to update expense form: {str(e)}")
            print(f"Error updating form: {e}")
            
    def validate_changes(self):
        """Validate form data before saving changes"""
